...
-(void)doRefresh
{
    UserListModel *mm = (UserListModel*)self.model;
    [mm clearCache:self.queryUserid sendType:self.sendtype];
    [mm load:TTURLRequestCachePolicyNoCache more:NO];
}

-(void)createModel{
        self.dataSource = [[UserListDataSource alloc] initWithSearchQuery:self.queryUserid sendType:self.sendtype];
}
